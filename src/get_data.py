...
def fetch_data(dataset):
    ...
    if dataset == "MIMIC":
        df_sepsis = run_query("src/sql_queries/mimic_table.sql",
                              "data/MIMIC_data.csv")

        df_icd = run_query("src/sql_queries/icd_MIMIC/icd_codes.sql",
                           "data/ICD_codes/MIMIC/raw_icd_codes.csv")

    elif dataset == "eICU":
        df_sepsis = run_query("src/sql_queries/eICU_table.sql",
                              "data/eICU_data.csv")

        df_icd = run_query("src/sql_queries/icd_eICU/icd_codes.sql",
                           "data/ICD_codes/eICU/raw_icd_codes.csv")

    return df_sepsis, df_icd
...
def parse_args():
    ...
    parser.add_argument("--dataset",
                    default="MIMIC",
                    help="Insert the dataset to work with, either MIMIC or eICU")
...
if __name__ == '__main__':
    ...
    df_sepsis, df_icd = fetch_data(args.dataset)
                  
    # 1. Convert ICD-9 to ICD-10 codes
    df_icd = icd_9_to_10(df_icd, args.dataset)
